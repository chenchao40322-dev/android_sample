...
public class FsDebugFileLog {
	 ...
	 */
	public static void init(String logPath, String logName, int logFileSizeLimit, int logListCountLimit) {
		mLogPath = logPath;
		mLogName = logName;
	...
	public synchronized static void b(String message) {
		try {
			...
			mLogList.add(logTime  + LOG_SPLITER + message + "\n");
		} catch (Exception e) {
	...
	/**
	 * getAllGupiao log items from @mLogList to @mWriteList,called by timer thread
	 */
	...
	private static void openLogFile(boolean deleteOld) {
		try {
			...
			Date date = new Date();
			SimpleDateFormat simpleDateFormate = new SimpleDateFormat("-yyyy_MM_dd_HH_mm_ss", Locale.getDefault());
			String logTime = simpleDateFormate.format(date);
			String realFilePath = mLogPath + "/" + mLogName + logTime + ".log";
	...
	public static void test() {
		FsDebugFileLog.init("/mnt/sdcard/funshion", "test");

		
		for (int i = 0; i < 1000; i= i+2) {
			FsDebugFileLog.b("test file log. index:" + i);
			FsDebugFileLog.b("test file log.text：中国  index:" + (i + 1));
		}
	}
}
